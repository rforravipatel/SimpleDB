...
public class BufferPool {
	...
	public Page getPage(TransactionId tid, PageId pid, Permissions perm)
		...
		// some code goes here
		if(pages.size() > numPages)                   //to check request for more than maximum pages in Bufferpool  
		{
			throw new DbException("");
		}else if(pages.containsKey(pid))              // return page if pid present
		{
			return page;
		}else                                         //getting page from DbFile
		{
			DbFile d = Database.getCatalog().getDbFile(pid.getTableId());
			return d.readPage(pid);
		}
