...
public class TupleDesc {
	...
	public String toString() throws UnsupportedOperationException{
		StringBuilder sb1 = new StringBuilder();                //to store entire string
		for(int i=0;i<types.length ;i++)                        //loop up to length of types array
		{
			sb1.append(types[i]).append("("+names[i]+")") ;//;  //concating the string in sb1
			if(i!=types.length-1)                               //to check the last element of types
			{
				sb1.append(", ");                               //to separate fieldType(fieldName)
			}
		}
		return sb1.toString();
	  	//throw new UnsupportedOperationException("Implement this");
